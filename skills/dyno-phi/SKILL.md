...
name: phi
description: >
  Use the `phi` CLI to upload binder candidate datasets, run the filtering pipeline
  (ProteinMPNN → ESMFold → AlphaFold2 → scoring), download results, and run research queries.
  Activate automatically when the user asks about phi commands, uploading PDB/CIF/FASTA files,
  filtering binder candidates, scoring designs, or downloading results.
---

# phi CLI skill

`phi` is the command-line interface for **Dyno Phi**, a filtering and ranking platform for
AI-generated protein binder candidates. It scores your designs with ProteinMPNN, ESMFold, and
AlphaFold2, applies experimentally calibrated thresholds, and returns a ranked shortlist.
...
|--------|-------|-----|------|------|------|
| `default` | ≥0.80 | ≥0.55 | ≥0.50 | ≤10.85 Å | ≤3.5 Å |
| `relaxed` | ≥0.80 | ≥0.45 | ≥0.50 | ≤12.4 Å | ≤4.5 Å |
...
```bash
# Default: single_sequence — recommended for truly novel de novo binders (no natural homologs)
phi filter --preset default --msa-tool single_sequence --wait

# mmseqs2: uses sequence homologs — better for sequences with known relatives
...
These are the individual steps executed automatically by `phi filter`. Use them only when
building custom pipelines or reusing existing intermediate results.
...
| `af2_ipae` | AlphaFold2 multimer | ≤ 10.85 Å |
